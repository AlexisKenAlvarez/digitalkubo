...
const Page = () => {
    ...
    const { register, setError, handleSubmit, formState: { errors } } = useForm<userInput>({ resolver: zodResolver(resetSchema) })
        ...
        <section className="w-full min-h-screen h-auto sm:py-4 py-7 relative">
            ...
            <div className="sm:w-fit w-full h-full sm:h-auto px-10 sm:py-7 sm:border-[1px] border-black/5 rounded-lg sm:mt-9 mt-4 drop-shadow-sm mx-auto">
                <div className="leading-loose text-center">
                    <h1 className="text-4xl sm:text-[48px] font-primary font-bold text-black">Password Reset</h1>
                </div>
                ...
                {sent ?
                    ...
                    : <div className="flex flex-col gap-y-3 mt-10">
                        ...
                        <Link href="/auth/login">
                            <h3 className="text-black/50 text-center">Back to <span className="font-bold text-button hover:text-nav cursor-pointer">Sign in</span></h3>
                        </Link>
