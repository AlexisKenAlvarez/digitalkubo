  ...
  callbacks: {
    async signIn({ user, account, profile, email, credentials }) {
      if (account && account.provider === "google") {
        ...
        const data = await axios.post(
          `${process.env.NEXTAUTH_URL}/api/checkEmail`,
          {
            email: user.email,
          }
        );
        ...
        if (!data.data.found) {
          const data = await axios.post(
            `${process.env.NEXTAUTH_URL}/api/addUser`,
            {
              email: user.email,
              password: null,
              accountType: 2,
        ...
        if (data.data.accountType === 2) {
          return true;
        } else if (data.data.accountType === 1) {
          // IF IT EXIST BUT NOT GOOGLE ACCOUNT, THEN ACCESS DENY, MUST USE PASSWORD INSTEAD
          throw new Error(
            `Login denied! &email=${user.email} &errorType=google-signin-denied`
          );
        }
